...
In the previous [post](http://haines-lab.com/2017/04/04/human-choice-and-reinforcement-learning-1/), I reviewed the Rescorla-Wagner updating (Delta) rule and its contemporary instantiation. At the end, I asked the following question: 
...
Here, we see that in the Delta rule the expected win probability for the next trial is equal to the [*exponentially weighted moving average*](https://en.wikipedia.org/wiki/Moving_average#Exponential_moving_average) of the past expectation and the current outcome. It is easy to show this through a visualization of the expectation over time. For example, imagine that we have a vector of outcomes $\lambda = [1,0,0,1,1,1,0,1,1,1]$, where 0 and 1 represent losing and winning slot machine rolls, respectively. Note that in this example, the placement of these outcomes within the vector $\lambda$ indicates their temporal order (i.e. $\lambda_{1}=1$, $\lambda_{2}=0$, etc.). Now, if we set an arbitrary learning rate such as $\beta = 0.05$, what is the expected win rate after iterating through outcomes $\lambda$? The R code below demonstrates the use of the Delta rule and an alternative exponential weighting scheme--which takes the form of a [*power series*](https://en.wikipedia.org/wiki/Power_series#Examples)--to determine the expectation on each trial:

```{r, 2017-04-07_fig1, out.extra="style='height: 100%; width: 100%; object-fit: contain'"}
# For pretty plots
library(ggplot2)
...
Through this example, those familiar with [economics](http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:moving_averages) or [signal processing](https://en.wikipedia.org/wiki/Signal_processing) may find the Delta rule familiar. Essentially, we can think of the Delta rule as a smoothing function or a [high- or low-pass filter](https://en.wikipedia.org/wiki/High-pass_filter#Algorithmic_implementation)--albeit in the time as opposed to frequency domain--which effectively attenuates the effect of past or current outcomes, respectively. What makes this specific form interesting is again the fact that it can be iterated (i.e. it is recursive), making it a realistic approximation to the computations performed by the brain when estimating some value.

With the above intuitions in mind, we will now get back to the question. How do we change the learning rate to ensure that the current expectation is always the simple average of all past outcomes? Since the above example showed that the Delta rule is really just a moving average where past outcomes are given exponentially decreasing weights, our goal is to make all outcomes equally represented. In other words, we want to weight past and current outcomes equally--this is a [*cumulative moving average*](https://en.wikipedia.org/wiki/Moving_average#Cumulative_moving_average). Using our slot machine example, the cumulative moving average formula (in its most common form) is written as follows: 
...
<img src="http://az616578.vo.msecnd.net/files/2016/08/19/636072180885324217301014601_shoes.jpg" alt="Figure 1" style="height: 100%; width: 100%; object-fit: contain"/>
...
Also known as the [Luce choice rule](https://en.wikipedia.org/wiki/Luce%27s_choice_axiom), the softmax allows choices to be probabilistic with weights respective to expected value:
...
```{r, 2017-04-07_fig2, out.extra="style='height: 100%; width: 100%; object-fit: contain'"}
