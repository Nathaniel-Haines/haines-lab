...
msg "Grabbing one file from the \`$SOURCE\` branch so that a commit can 
be made"
...
msg "Removing the \`public\` folder to make room for the \`master\` 
subtree"
rm -rf public
git add -u
...
git subtree add --prefix=public \
    git@github.com:$USERNAME/haines-lab.git master --squash

msg "Pulling down the just committed file to help avoid merge conflicts"
git subtree pull --prefix=public \
    git@github.com:$USERNAME/haines-lab.git master
